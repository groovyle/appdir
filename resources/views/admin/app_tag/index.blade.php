...
@section('content')
	...
	<div class="card main-content @if($show_filters || request()->has('page')) scroll-to-me @endif">
		...
		@if($list->isEmpty())
		<div class="card-body">
			<h4 class="text-left">{{ __('admin/app_tags.no_tags_yet') }}</h4>
		</div>
		@else
					...
					<tbody>
						...
						<tr class="tag-item-{{ \Str::slug($item->name) }}">
							...
							<td class="text-nowrap">
								<a href="{{ route('admin.app_tags.show', ['tag' => $item->name]) }}" class="btn btn-default btn-xs text-nowrap btn-ofa-modal" data-title="{{ __('admin/app_tags.page_title.detail') }}: {{ text_truncate($item->name, 30) }}" data-size="sm" data-footer="false">
