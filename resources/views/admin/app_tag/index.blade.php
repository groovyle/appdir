...
@section('title')
{{ __('admin/app_tags.page_title.index') }} - @parent
@endsection

@section('page-title')
{{ __('admin/app_tags.page_title.index') }}
<span class="page-sub-title">{{ __('common.total_x', ['x' => $total]) }}</span>
@endsection

@section('content')
	<div class="mt-2 mb-3">
		@can('create', App\Models\AppTag::class)
		<a href="{{ route('admin.app_tags.create') }}" class="btn btn-primary">{{ __('admin/app_tags.add_new_tag') }}</a>
		@endcan
	</div>

	<!-- Filters -->
	<form class="card card-primary card-outline filters-wrapper @if($hide_filters) collapsed-card @endif" method="GET" action="{{ route('admin.app_tags.index') }}">
			...
			<div class="form-horizontal">
				...
				<div class="form-group row mb-0">
					<div class="offset-sm-3 offset-lg-2 col">
						...
						<a class="btn btn-secondary btn-sm ml-2" href="{{ route('admin.app_tags.index') }}">{{ __('admin/common.reset') }}</a>
					</div>
				</div>
			</div>
		</div>
	...
	<div class="card main-content @if($show_filters || request()->has('page')) scroll-to-me @endif">
		<div class="card-header">
			<h3 class="card-title">{{ __('admin/app_tags.tags_list') }} ({{ $list->total() }})</h3>
			<div class="card-tools">
		...
		<div class="card-body">
			@if($total == 0)
			<h4 class="text-left">&ndash; {{ __('admin/app_tags.no_tags_yet') }} &ndash;</h4>
			@else
			<h5 class="text-left">&ndash; {{ __('admin/app_tags.no_tags_matches') }} &ndash;</h5>
			@endif
		</div>
		@else
				...
				<table class="table table-head-fixed table-hover table-sm">
					<thead>
						<tr>
							...
							<th class="text-center @if($filters['sort_by'] == 'apps') text-primary @endif" style="width: 20%;">{{ __('admin/common.fields.number_of_apps') }}</th>
							<th style="width: 1%;">{{ __('common.actions') }}</th>
						</tr>
					</thead>
					<tbody>
						@foreach($list as $item)
						<tr class="tag-item-{{ \Str::slug($item->name) }}">
							...
							<td class="text-nowrap">
								@can('view', $item)
								<a href="{{ route('admin.app_tags.show', ['tag' => $item->name]) }}" class="btn btn-default btn-xs text-nowrap btn-ofa-modal" data-title="{{ __('admin/app_tags.page_title.detail') }}: {{ text_truncate($item->name, 30) }}" data-footer="false">
									<span class="fas fa-search mr-1"></span>
									{{ __('common.view') }}
								</a>
								@endcan
								@can('update', $item)
								<a href="{{ route('admin.app_tags.edit', ['tag' => $item->name, 'backto' => 'list']) }}" class="btn btn-primary btn-xs text-nowrap">
									<span class="fas fa-edit mr-1"></span>
									{{ __('common.edit') }}
								</a>
								@endcan
								@can('delete', $item)
								<a href="{{ route('admin.app_tags.destroy', ['tag' => $item->name, 'backto' => 'back']) }}" class="btn btn-danger btn-xs text-nowrap btn-ays-modal" data-method="DELETE" data-prompt="_delete" data-description="{{ sprintf('<strong>%s</strong>: %s', __('admin/app_tags._self'), $item->name) }}">
...
jQuery(document).ready(function($) {
	...
	@isset($goto_item)
	var $scrollTarget = $(@json('.tag-item-'.\Str::slug($goto_item)));
