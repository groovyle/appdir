...
@section('title')
{{ __('admin/app_categories.page_title.index') }} - @parent
@endsection

@section('page-title')
{{ __('admin/app_categories.page_title.index') }}
<span class="page-sub-title">{{ __('common.total_x', ['x' => $total]) }}</span>
@endsection

@section('content')
	<div class="mt-2 mb-3">
		@can('create', App\Models\AppCategory::class)
		<a href="{{ route('admin.app_categories.create') }}" class="btn btn-primary">{{ __('admin/app_categories.add_new_category') }}</a>
		@endcan
	</div>

	<!-- Filters -->
	<form class="card card-primary card-outline filters-wrapper @if($hide_filters) collapsed-card @endif" method="GET" action="{{ route('admin.app_categories.index') }}">
			...
			<div class="form-horizontal">
				...
				<div class="form-group row mb-0">
					<div class="offset-sm-3 offset-lg-2 col">
						...
						<a class="btn btn-secondary btn-sm ml-2" href="{{ route('admin.app_categories.index') }}">{{ __('admin/common.reset') }}</a>
					</div>
				</div>
			</div>
		</div>
	...
	<div class="card main-content @if($scroll_content) scroll-to-me @endif">
		<div class="card-header">
			<h3 class="card-title">{{ __('admin/app_categories.categories_list') }} ({{ $list->total() }})</h3>
			<div class="card-tools">
			...
			<div class="table-responsive">
				<table class="table table-head-fixed table-hover table-sm">
					...
					<tbody>
						...
						<tr class="category-item-{{ $item->id }}">
							...
							<td class="text-nowrap">
								...
								@endcan
								@can('update', $item)
								<a href="{{ route('admin.app_categories.edit', ['cat' => $item->id, 'backto' => 'list']) }}" class="btn btn-primary btn-xs text-nowrap">
									<span class="fas fa-edit mr-1"></span>
									{{ __('common.edit') }}
								</a>
								@endcan
								@can('delete', $item)
								<a href="{{ route('admin.app_categories.destroy', ['cat' => $item->id, 'backto' => 'back']) }}" class="btn btn-danger btn-xs text-nowrap btn-ays-modal" data-method="DELETE" data-prompt="_delete" data-description="{{ sprintf('<strong>%s</strong>: %s (%s: %s)', __('admin/app_categories._self'), $item->name, __('admin/common.fields.id'), $item->id) }}">
...
jQuery(document).ready(function($) {
	...
	@isset($goto_item)
	var $scrollTarget = $(@json('.category-item-'.$goto_item));
